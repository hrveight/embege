...
# Initialize session state variables to store analysis results
...
if 'cluster_method' not in st.session_state:
    st.session_state.cluster_method = "average"
...
def run_clustering_analysis(df_merged, selected_vars, cluster_method, num_clusters):
    ...
    return df_clean, clusters, linkage_matrix, data_norm, silhouette_avg, Sw, Sb, ratio, centroids, pca, pca_result, pca_df
        ...
        if len(selected_vars) < 2:
            ...
            with st.spinner("Melakukan clustering..."):
                (st.session_state.df_clean, 
                 ...
                 st.session_state.data_norm,
                 st.session_state.silhouette_avg, 
                 st.session_state.Sw, 
                 st.session_state.Sb, 
                 ...
                 st.session_state.pca_df) = run_clustering_analysis(
                    df_merged, 
                    selected_vars, 
                    cluster_method, 
                    num_clusters
            ...
            st.experimental_rerun()  # Rerun to display results with updated session state

    # Display results if analysis has been run
    if st.session_state.analysis_run:
        # Display number of records after cleaning
        st.write(f"Data bersih: {st.session_state.df_clean.shape[0]} wilayah (setelah menghapus missing values)")
        ...
        with tab1:
            ...
            # Show cluster counts
            cluster_counts = st.session_state.df_clean['Cluster'].value_counts().sort_index()
        ...
        with tab2:
            ...
            plt.title(f"Dendrogram (Metode {st.session_state.cluster_method.capitalize()})")
            plt.xlabel("Index")
            plt.ylabel("Distance")
            plt.tight_layout()
            st.pyplot(fig)

            # Option to show full dendrogram - uses a checkbox with a unique key
            if st.checkbox("Tampilkan dendrogram lengkap", key="show_full_dendrogram"):
                # This block will run when checkbox is checked, without losing analysis state
                with st.spinner("Menggambar dendrogram lengkap..."):
                    fig, ax = plt.subplots(figsize=(20, 10))
                    dendrogram(
                        st.session_state.linkage_matrix,
                        labels=st.session_state.df_clean['Kode Wilayah'].astype(str).values,
                        leaf_rotation=90,
                        ax=ax
                    )
                    plt.title(f"Dendrogram Lengkap (Metode {st.session_state.cluster_method.capitalize()})")
        ...
        with tab3:
            ...
            # Use PCA results from session state
            ...
            fig = px.scatter(
                ...
                title=f"PCA - Visualisasi Cluster ({st.session_state.cluster_method.capitalize()})",
            ...
            loading_scores = pd.DataFrame(
                ...
                columns=['PC1', 'PC2'],
                index=st.session_state.selected_vars
            )
        ...
        with tab4:
            ...
            with col1:
                st.metric("Silhouette Score", f"{st.session_state.silhouette_avg:.4f}")
            with col2:
                st.metric("Within-cluster variation (Sw)", f"{st.session_state.Sw:.4f}" if not np.isnan(st.session_state.Sw) else "N/A")
            with col3:
                st.metric("Between-cluster variation (Sb)", f"{st.session_state.Sb:.4f}" if not np.isnan(st.session_state.Sb) else "N/A")

            st.write(f"Sw/Sb Ratio: {st.session_state.ratio:.4f}" if not np.isnan(st.session_state.ratio) else "N/A")
            ...
            # Use a unique key for this button to maintain state
            if st.button("Jalankan Bootstrap Validation", key="run_bootstrap_btn"):
                with st.spinner("Menjalankan Bootstrap... (ini mungkin membutuhkan waktu)"):
                    ...
                    # Original clustering
                    orig_clusters = st.session_state.clusters
                    ...
                    for i in range(n_bootstrap):
                        # Bootstrap resample
                        bootstrap_indices = resample(range(len(st.session_state.data_norm)), replace=True, n_samples=len(st.session_state.data_norm))
                        bootstrap_data = st.session_state.data_norm.iloc[bootstrap_indices]

                        # Compute linkage for bootstrap sample
                        try:
                            bootstrap_linkage = linkage(bootstrap_data, method=st.session_state.cluster_method)

                            # Form clusters
                            bootstrap_clusters = fcluster(bootstrap_linkage, t=st.session_state.num_clusters, criterion='maxclust')

                            # Map bootstrap clusters back to original indices
                            mapped_clusters = np.zeros(len(st.session_state.data_norm))
        ...
        with tab5:
            ...
            for cluster_id in sorted(st.session_state.df_clean['Cluster'].unique()):
                with st.expander(f"Cluster {cluster_id} ({(st.session_state.df_clean['Cluster'] == cluster_id).sum()} wilayah)"):
                    cluster_data = st.session_state.df_clean[st.session_state.df_clean['Cluster'] == cluster_id]
                    ...
                    overall_mean = st.session_state.df_clean[st.session_state.selected_vars].mean()
                    overall_std = st.session_state.df_clean[st.session_state.selected_vars].std()

                    cluster_mean = cluster_data[st.session_state.selected_vars].mean()
            ...
            csv = convert_df(st.session_state.df_clean)
            st.download_button(
